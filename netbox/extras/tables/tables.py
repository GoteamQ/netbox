...
class CustomFieldTable(NetBoxTable):
    ...
    object_types = columns.ContentTypesColumn(verbose_name=_('Object Types'))
    required = columns.BooleanColumn(verbose_name=_('Required'), false_mark=None)
    unique = columns.BooleanColumn(verbose_name=_('Validate Uniqueness'), false_mark=None)
    ui_visible = columns.ChoiceFieldColumn(verbose_name=_('Visible'))
    ui_editable = columns.ChoiceFieldColumn(verbose_name=_('Editable'))
    description = columns.MarkdownColumn(verbose_name=_('Description'))
    related_object_type = columns.ContentTypeColumn(verbose_name=_('Related Object Type'))
    choice_set = tables.Column(linkify=True, verbose_name=_('Choice Set'))
    choices = columns.ChoicesColumn(max_items=10, orderable=False, verbose_name=_('Choices'))
    is_cloneable = columns.BooleanColumn(verbose_name=_('Is Cloneable'), false_mark=None)
...
class CustomFieldChoiceSetTable(NetBoxTable):
    ...
    choices = columns.ChoicesColumn(max_items=10, orderable=False)
    choice_count = tables.TemplateColumn(
        accessor=tables.A('extra_choices'), template_code='{{ value|length }}', orderable=False, verbose_name=_('Count')
    )
    order_alphabetically = columns.BooleanColumn(verbose_name=_('Order Alphabetically'), false_mark=None)
    owner = tables.Column(linkify=True, verbose_name=_('Owner'))
...
class CustomLinkTable(NetBoxTable):
    ...
    new_window = columns.BooleanColumn(verbose_name=_('New Window'), false_mark=None)
    owner = tables.Column(linkify=True, verbose_name=_('Owner'))
...
class ExportTemplateTable(NetBoxTable):
    ...
    mime_type = tables.Column(verbose_name=_('MIME Type'))
    ...
    as_attachment = columns.BooleanColumn(verbose_name=_('As Attachment'), false_mark=None)
    data_source = tables.Column(verbose_name=_('Data Source'), linkify=True)
    data_file = tables.Column(verbose_name=_('Data File'), linkify=True)
    is_synced = columns.BooleanColumn(orderable=False, verbose_name=_('Synced'))
    owner = tables.Column(linkify=True, verbose_name=_('Owner'))
...
class ImageAttachmentTable(NetBoxTable):
    id = tables.Column(verbose_name=_('ID'), linkify=False)
    image = columns.TemplateColumn(
        verbose_name=_('Image'), template_code=IMAGEATTACHMENT_IMAGE, attrs={'td': {'class': 'text-nowrap'}}
    )
    name = tables.Column(
        verbose_name=_('Name'),
        linkify=True,
    ...
    size = tables.Column(orderable=False, verbose_name=_('Size'))
...
class SavedFilterTable(NetBoxTable):
    ...
    shared = columns.BooleanColumn(verbose_name=_('Shared'), false_mark=None)
    owner = tables.Column(linkify=True, verbose_name=_('Owner'))
...
class TableConfigTable(NetBoxTable):
    ...
    table = tables.Column(verbose_name=_('Table Name'))
    enabled = columns.BooleanColumn(
        verbose_name=_('Enabled'),
    )
    shared = columns.BooleanColumn(verbose_name=_('Shared'), false_mark=None)
...
class NotificationTable(NetBoxTable):
    ...
    object = columns.TemplateColumn(verbose_name=_('Object'), template_code=NOTIFICATION_LINK, orderable=False)
    ...
    user = tables.Column(verbose_name=_('User'), linkify=True)
    actions = NotificationActionsColumn(actions=('dismiss',))
...
class NotificationGroupTable(NetBoxTable):
    ...
    users = columns.ManyToManyColumn(linkify_item=True)
...
class WebhookTable(NetBoxTable):
    name = tables.Column(verbose_name=_('Name'), linkify=True)
    ssl_validation = columns.BooleanColumn(verbose_name=_('SSL Validation'))
    owner = tables.Column(linkify=True, verbose_name=_('Owner'))
    tags = columns.TagColumn(url_name='extras:webhook_list')
...
class EventRuleTable(NetBoxTable):
    ...
    event_types = columns.ArrayColumn(verbose_name=_('Event Types'), func=get_event_text, orderable=False)
    owner = tables.Column(linkify=True, verbose_name=_('Owner'))
    tags = columns.TagColumn(url_name='extras:webhook_list')
...
class ConfigContextProfileTable(PrimaryModelTable):
    ...
    data_file = tables.Column(verbose_name=_('Data File'), linkify=True)
    is_synced = columns.BooleanColumn(orderable=False, verbose_name=_('Synced'))
    tags = columns.TagColumn(url_name='extras:configcontextprofile_list')
...
class ConfigContextTable(NetBoxTable):
    ...
    is_synced = columns.BooleanColumn(orderable=False, verbose_name=_('Synced'))
    owner = tables.Column(linkify=True, verbose_name=_('Owner'))
    tags = columns.TagColumn(url_name='extras:configcontext_list')
...
class ConfigTemplateTable(NetBoxTable):
    ...
    as_attachment = columns.BooleanColumn(verbose_name=_('As Attachment'), false_mark=None)
    owner = tables.Column(linkify=True, verbose_name=_('Owner'))
    tags = columns.TagColumn(url_name='extras:configtemplate_list')
    role_count = columns.LinkedCountColumn(
        viewname='dcim:devicerole_list', url_params={'config_template_id': 'pk'}, verbose_name=_('Device Roles')
    )
    platform_count = columns.LinkedCountColumn(
        viewname='dcim:platform_list', url_params={'config_template_id': 'pk'}, verbose_name=_('Platforms')
    )
    device_count = columns.LinkedCountColumn(
        viewname='dcim:device_list', url_params={'config_template_id': 'pk'}, verbose_name=_('Devices')
    )
...
class JournalEntryTable(NetBoxTable):
    created = columns.DateTimeColumn(verbose_name=_('Created'), timespec='minutes', linkify=True)
    ...
    class Meta(NetBoxTable.Meta):
        ...
        default_columns = ('pk', 'created', 'created_by', 'assigned_object_type', 'assigned_object', 'kind', 'comments')


class ScriptResultsTable(BaseTable):
    index = tables.Column(verbose_name=_('Line'))
    time = columns.DateTimeColumn(verbose_name=_('Time'), timespec='seconds')
    ...
    message = columns.MarkdownColumn(verbose_name=_('Message'))
...
class ReportResultsTable(BaseTable):
    ...
    time = tables.Column(verbose_name=_('Time'))
    status = tables.TemplateColumn(
        template_code="""{% load log_levels %}{% log_level record.status %}""", verbose_name=_('Level')
    )
    object = tables.Column(verbose_name=_('Object'))
    url = tables.Column(verbose_name=_('URL'))
    message = columns.MarkdownColumn(verbose_name=_('Message'))
