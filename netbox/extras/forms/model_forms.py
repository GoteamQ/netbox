...
class CustomFieldForm(ChangelogMessageMixin, OwnerMixin, forms.ModelForm):
    ...
    choice_set = DynamicModelChoiceField(queryset=CustomFieldChoiceSet.objects.all())
    ...
    def __init__(self, *args, **kwargs):
        ...
        if field_type in (CustomFieldTypeChoices.TYPE_SELECT, CustomFieldTypeChoices.TYPE_MULTISELECT):
            self.fieldsets = (self.fieldsets[0], FieldSet('choice_set', name=_('Choices')), *self.fieldsets[1:])
        else:
...
class CustomLinkForm(ChangelogMessageMixin, OwnerMixin, forms.ModelForm):
    object_types = ContentTypeMultipleChoiceField(
        label=_('Object types'), queryset=ObjectType.objects.with_feature('custom_links')
    )
    ...
    class Meta:
        ...
        help_texts = {
            ...
            'link_url': _('Jinja2 template code for the link URL. Reference the object as {example}.').format(
...
class ExportTemplateForm(ChangelogMessageMixin, SyncedDataMixin, OwnerMixin, forms.ModelForm):
    object_types = ContentTypeMultipleChoiceField(
        label=_('Object types'), queryset=ObjectType.objects.with_feature('export_templates')
    )
    template_code = forms.CharField(
        label=_('Template code'), required=False, widget=forms.Textarea(attrs={'class': 'font-monospace'})
    )
...
class SavedFilterForm(ChangelogMessageMixin, OwnerMixin, forms.ModelForm):
    slug = SlugField()
    object_types = ContentTypeMultipleChoiceField(label=_('Object types'), queryset=ObjectType.objects.all())
...
class TableConfigForm(forms.ModelForm):
    object_type = ContentTypeChoiceField(label=_('Object type'), queryset=ObjectType.objects.all())
...
class BookmarkForm(forms.ModelForm):
    object_type = ContentTypeChoiceField(label=_('Object type'), queryset=ObjectType.objects.with_feature('bookmarks'))
...
class NotificationGroupForm(ChangelogMessageMixin, forms.ModelForm):
    ...
    users = DynamicModelMultipleChoiceField(label=_('Users'), required=False, queryset=User.objects.all())
...
class SubscriptionForm(forms.ModelForm):
    object_type = ContentTypeChoiceField(
        label=_('Object type'), queryset=ObjectType.objects.with_feature('notifications')
    )
...
class EventRuleForm(OwnerMixin, NetBoxModelForm):
    ...
    event_types = forms.MultipleChoiceField(choices=get_event_type_choices(), label=_('Event types'))
    action_choice = forms.ChoiceField(label=_('Action choice'), choices=[])
    conditions = JSONField(
        required=False, help_text=_('Enter conditions in <a href="https://json.org/">JSON</a> format.')
    )
    ...
    def init_script_choice(self):
        ...
        self.fields['action_choice'] = DynamicModelChoiceField(
            label=_('Script'), queryset=Script.objects.all(), required=True, initial=initial
        )
    ...
    def init_webhook_choice(self):
        ...
        self.fields['action_choice'] = DynamicModelChoiceField(
            label=_('Webhook'), queryset=Webhook.objects.all(), required=True, initial=initial
        )
    ...
    def init_notificationgroup_choice(self):
        ...
        self.fields['action_choice'] = DynamicModelChoiceField(
            label=_('Notification group'), queryset=NotificationGroup.objects.all(), required=True, initial=initial
        )
    ...
    def clean(self):
        ...
        # Script
            ...
            self.cleaned_data['action_object_type'] = ObjectType.objects.get_for_model(Script, for_concrete_model=False)
            self.cleaned_data['action_object_id'] = action_choice.id
        # Notification
...
class TagForm(ChangelogMessageMixin, OwnerMixin, forms.ModelForm):
    ...
    fieldsets = (FieldSet('name', 'slug', 'color', 'weight', 'description', 'object_types', name=_('Tag')),)
...
class ConfigContextForm(ChangelogMessageMixin, SyncedDataMixin, OwnerMixin, forms.ModelForm):
    profile = DynamicModelChoiceField(label=_('Profile'), queryset=ConfigContextProfile.objects.all(), required=False)
    ...
    cluster_types = DynamicModelMultipleChoiceField(
        label=_('Cluster types'), queryset=ClusterType.objects.all(), required=False
    )
    cluster_groups = DynamicModelMultipleChoiceField(
        label=_('Cluster groups'), queryset=ClusterGroup.objects.all(), required=False
    )
    clusters = DynamicModelMultipleChoiceField(label=_('Clusters'), queryset=Cluster.objects.all(), required=False)
    ...
    tags = DynamicModelMultipleChoiceField(label=_('Tags'), queryset=Tag.objects.all(), required=False)
    data = JSONField(label=_('Data'), required=False)
...
class ConfigTemplateForm(ChangelogMessageMixin, SyncedDataMixin, OwnerMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        fields = '__all__'
        widgets = {'environment_params': forms.Textarea(attrs={'rows': 5})}
...
class ImageAttachmentForm(forms.ModelForm):
    fieldsets = (FieldSet(ObjectAttribute('parent'), 'image', 'name', 'description'),)
...
class JournalEntryForm(NetBoxModelForm):
    kind = forms.ChoiceField(label=_('Kind'), choices=JournalEntryKindChoices)
