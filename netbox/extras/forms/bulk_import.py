...
class SavedFilterImportForm(OwnerCSVMixin, CSVModelForm):
    object_types = CSVMultipleContentTypeField(
        label=_('Object types'), queryset=ObjectType.objects.all(), help_text=_('One or more assigned object types')
    )
...
class EventRuleImportForm(OwnerCSVMixin, NetBoxModelImportForm):
    ...
    action_object = forms.CharField(
        label=_('Action object'), required=True, help_text=_('Webhook name or script as dotted path module.Class')
    )
...
class JournalEntryImportForm(NetBoxModelImportForm):
    ...
    kind = CSVChoiceField(label=_('Kind'), choices=JournalEntryKindChoices, help_text=_('The classification of entry'))
    comments = forms.CharField(label=_('Comments'), required=True)

    class Meta:
        model = JournalEntry
        fields = ('assigned_object_type', 'assigned_object_id', 'created_by', 'kind', 'comments', 'tags')
