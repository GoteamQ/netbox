...
class CustomFieldChoiceSetFilterForm(OwnerFilterMixin, SavedFiltersMixin, FilterForm):
    ...
    base_choices = forms.MultipleChoiceField(choices=CustomFieldChoiceSetBaseChoices, required=False)
...
class CustomLinkFilterForm(OwnerFilterMixin, SavedFiltersMixin, FilterForm):
    ...
    new_window = forms.NullBooleanField(
        label=_('New window'), required=False, widget=forms.Select(choices=BOOLEAN_WITH_BLANK_CHOICES)
    )
    weight = forms.IntegerField(label=_('Weight'), required=False)
...
class ExportTemplateFilterForm(OwnerFilterMixin, SavedFiltersMixin, FilterForm):
    ...
    data_file_id = DynamicModelMultipleChoiceField(
        ...
        required=False,
        label=_('Data file'),
        query_params={'source_id': '$data_source_id'},
    )
    object_type_id = ContentTypeMultipleChoiceField(
        queryset=ObjectType.objects.with_feature('export_templates'), required=False, label=_('Content types')
    )
    mime_type = forms.CharField(required=False, label=_('MIME type'))
    file_name = forms.CharField(label=_('File name'), required=False)
    file_extension = forms.CharField(label=_('File extension'), required=False)
    as_attachment = forms.NullBooleanField(
        label=_('As attachment'), required=False, widget=forms.Select(choices=BOOLEAN_WITH_BLANK_CHOICES)
    )
...
class ImageAttachmentFilterForm(SavedFiltersMixin, FilterForm):
    ...
    object_type_id = ContentTypeChoiceField(
        label=_('Object type'), queryset=ObjectType.objects.with_feature('image_attachments'), required=False
    )
...
class SavedFilterFilterForm(OwnerFilterMixin, SavedFiltersMixin, FilterForm):
    ...
    shared = forms.NullBooleanField(
        label=_('Shared'), required=False, widget=forms.Select(choices=BOOLEAN_WITH_BLANK_CHOICES)
    )
    weight = forms.IntegerField(label=_('Weight'), required=False)
...
class TableConfigFilterForm(SavedFiltersMixin, FilterForm):
    ...
    object_type_id = ContentTypeMultipleChoiceField(
        label=_('Object types'), queryset=ObjectType.objects.public(), required=False
    )
    enabled = forms.NullBooleanField(
        label=_('Enabled'), required=False, widget=forms.Select(choices=BOOLEAN_WITH_BLANK_CHOICES)
    )
...
class WebhookFilterForm(OwnerFilterMixin, NetBoxModelFilterSetForm):
    ...
    http_content_type = forms.CharField(label=_('HTTP content type'), required=False)
    payload_url = forms.CharField(label=_('Payload URL'), required=False)
    http_method = forms.MultipleChoiceField(choices=WebhookHttpMethodChoices, required=False, label=_('HTTP method'))
...
class EventRuleFilterForm(OwnerFilterMixin, NetBoxModelFilterSetForm):
    ...
    object_type_id = ContentTypeMultipleChoiceField(
        queryset=ObjectType.objects.with_feature('event_rules'), required=False, label=_('Object type')
    )
    event_type = forms.MultipleChoiceField(choices=get_event_type_choices, required=False, label=_('Event type'))
    action_type = forms.ChoiceField(
        choices=add_blank_choice(EventRuleActionChoices), required=False, label=_('Action type')
    )
    enabled = forms.NullBooleanField(
        label=_('Enabled'), required=False, widget=forms.Select(choices=BOOLEAN_WITH_BLANK_CHOICES)
    )
...
class ConfigContextFilterForm(OwnerFilterMixin, SavedFiltersMixin, FilterForm):
    ...
    profile_id = DynamicModelMultipleChoiceField(
        queryset=ConfigContextProfile.objects.all(), required=False, label=_('Profile')
    )
    data_source_id = DynamicModelMultipleChoiceField(
        queryset=DataSource.objects.all(), required=False, label=_('Data source')
    )
    ...
    region_id = DynamicModelMultipleChoiceField(queryset=Region.objects.all(), required=False, label=_('Regions'))
    site_group_id = DynamicModelMultipleChoiceField(
        queryset=SiteGroup.objects.all(), required=False, label=_('Site groups')
    )
    site_id = DynamicModelMultipleChoiceField(queryset=Site.objects.all(), required=False, label=_('Sites'))
    location_id = DynamicModelMultipleChoiceField(queryset=Location.objects.all(), required=False, label=_('Locations'))
    device_type_id = DynamicModelMultipleChoiceField(
        queryset=DeviceType.objects.all(), required=False, label=_('Device types')
    )
    device_role_id = DynamicModelMultipleChoiceField(
        queryset=DeviceRole.objects.all(), required=False, label=_('Roles')
    )
    platform_id = DynamicModelMultipleChoiceField(queryset=Platform.objects.all(), required=False, label=_('Platforms'))
    cluster_type_id = DynamicModelMultipleChoiceField(
        queryset=ClusterType.objects.all(), required=False, label=_('Cluster types')
    )
    cluster_group_id = DynamicModelMultipleChoiceField(
        queryset=ClusterGroup.objects.all(), required=False, label=_('Cluster groups')
    )
    cluster_id = DynamicModelMultipleChoiceField(queryset=Cluster.objects.all(), required=False, label=_('Clusters'))
    ...
    tag_id = DynamicModelMultipleChoiceField(queryset=Tag.objects.all(), required=False, label=_('Tags'))
...
class ConfigTemplateFilterForm(OwnerFilterMixin, SavedFiltersMixin, FilterForm):
    ...
    auto_sync_enabled = forms.NullBooleanField(
        label=_('Auto sync enabled'), required=False, widget=forms.Select(choices=BOOLEAN_WITH_BLANK_CHOICES)
    )
...
class JournalEntryFilterForm(NetBoxModelFilterSetForm):
    ...
    created_after = forms.DateTimeField(required=False, label=_('After'), widget=DateTimePicker())
    created_before = forms.DateTimeField(required=False, label=_('Before'), widget=DateTimePicker())
    created_by_id = DynamicModelMultipleChoiceField(queryset=User.objects.all(), required=False, label=_('User'))
...
class NotificationGroupFilterForm(SavedFiltersMixin, FilterForm):
    ...
    group_id = DynamicModelMultipleChoiceField(queryset=Group.objects.all(), required=False, label=_('Group'))
