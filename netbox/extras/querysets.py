...
class ConfigContextQuerySet(RestrictedQuerySet):
    def get_for_object(self, obj, aggregate_data=False):
        ...
        if aggregate_data:
            return queryset.aggregate(config_context_data=JSONBAgg('data', ordering=['weight', 'name']))[
                'config_context_data'
            ]
    ...
    def annotate_config_context_data(self):
        ...
        return self.annotate(
            config_context_data=Subquery(
                ...
                .annotate(_data=EmptyGroupByJSONBAgg('data', order_by=['weight', 'name']))
                .values('_data')
                .order_by()
            )
        )
        ...
        base_query = Q(
            ...
            Q(
                tags__pk__in=Subquery(
                    TaggedItem.objects.filter(**tag_query_filters).values_list('tag_id', flat=True).distinct()
                )
            )
            | Q(tags=None),
            is_active=True,
