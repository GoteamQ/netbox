...
class L2VPNTable(TenancyColumnsMixin, ContactsColumnMixin, PrimaryModelTable):
    ...
    import_targets = columns.TemplateColumn(
        verbose_name=_('Import Targets'), template_code=L2VPN_TARGETS, orderable=False
    )
    export_targets = columns.TemplateColumn(
        verbose_name=_('Export Targets'), template_code=L2VPN_TARGETS, orderable=False
    )
    tags = columns.TagColumn(url_name='vpn:l2vpn_list')
...
class L2VPNTerminationTable(NetBoxTable):
    pk = columns.ToggleColumn()
    l2vpn = tables.Column(verbose_name=_('L2VPN'), linkify=True)
    ...
    assigned_object_parent = tables.Column(linkify=True, orderable=False, verbose_name=_('Object Parent'))
    assigned_object_site = tables.Column(linkify=True, orderable=False, verbose_name=_('Object Site'))
    tags = columns.TagColumn(url_name='vpn:l2vpntermination_list')
