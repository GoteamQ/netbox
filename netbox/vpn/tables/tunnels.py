...
class TunnelGroupTable(ContactsColumnMixin, OrganizationalModelTable):
    ...
    tunnel_count = columns.LinkedCountColumn(
        viewname='vpn:tunnel_list', url_params={'group_id': 'pk'}, verbose_name=_('Tunnels')
    )
    tags = columns.TagColumn(url_name='vpn:tunnelgroup_list')
...
class TunnelTable(TenancyColumnsMixin, ContactsColumnMixin, PrimaryModelTable):
    ...
    ipsec_profile = tables.Column(verbose_name=_('IPSec profile'), linkify=True)
    ...
    tags = columns.TagColumn(url_name='vpn:tunnel_list')
...
class TunnelTerminationTable(NetBoxTable):
    tunnel = tables.Column(verbose_name=_('Tunnel'), linkify=True)
    role = columns.ChoiceFieldColumn(verbose_name=_('Role'))
    termination_parent = tables.Column(
        accessor='termination__parent_object', linkify=True, orderable=False, verbose_name=_('Host')
    )
    ...
    outside_ip = tables.Column(verbose_name=_('Outside IP'), linkify=True)
    tags = columns.TagColumn(url_name='vpn:tunneltermination_list')
