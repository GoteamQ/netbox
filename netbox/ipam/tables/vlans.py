...
class VLANGroupTable(TenancyColumnsMixin, OrganizationalModelTable):
    ...
    vid_ranges_list = columns.ArrayColumn(accessor='vid_ranges_items', verbose_name=_('VID Ranges'), orderable=False)
    vlan_count = columns.LinkedCountColumn(
        viewname='ipam:vlan_list', url_params={'group_id': 'pk'}, verbose_name=_('VLANs')
    )
    ...
    class Meta(OrganizationalModelTable.Meta):
        ...
        default_columns = ('pk', 'name', 'scope_type', 'scope', 'vlan_count', 'utilization', 'tenant', 'description')
...
class VLANTable(TenancyColumnsMixin, PrimaryModelTable):
    ...
    qinq_role = columns.ChoiceFieldColumn(verbose_name=_('Q-in-Q role'))
...
class InterfaceVLANTable(NetBoxTable):
    ...
    tagged = columns.BooleanColumn(verbose_name=_('Tagged'), false_mark=None)
    site = tables.Column(verbose_name=_('Site'), linkify=True)
...
class VLANTranslationPolicyTable(PrimaryModelTable):
    ...
    rule_count = columns.LinkedCountColumn(
        viewname='ipam:vlantranslationrule_list', url_params={'policy_id': 'pk'}, verbose_name=_('Rules')
    )
    description = tables.Column(
        verbose_name=_('Description'),
    )
    tags = columns.TagColumn(url_name='ipam:vlantranslationpolicy_list')
...
class VLANTranslationRuleTable(NetBoxTable):
    policy = tables.Column(verbose_name=_('Policy'), linkify=True)
    local_vid = tables.Column(verbose_name=_('Local VID'), linkify=True)
    ...
    tags = columns.TagColumn(url_name='ipam:vlantranslationrule_list')
