...
class VRFBulkEditForm(PrimaryModelBulkEditForm):
    ...
    enforce_unique = forms.NullBooleanField(
        required=False, widget=BulkEditNullBooleanSelect(), label=_('Enforce unique space')
    )

    model = VRF
    fieldsets = (FieldSet('tenant', 'enforce_unique', 'description'),)
...
class RouteTargetBulkEditForm(PrimaryModelBulkEditForm):
    ...
    model = RouteTarget
    fieldsets = (FieldSet('tenant', 'description'),)
    nullable_fields = ('tenant', 'description', 'comments')
...
class RIRBulkEditForm(OrganizationalModelBulkEditForm):
    ...
    model = RIR
    fieldsets = (FieldSet('is_private', 'description'),)
...
class ASNRangeBulkEditForm(OrganizationalModelBulkEditForm):
    ...
    model = ASNRange
    fieldsets = (FieldSet('rir', 'tenant', 'description'),)
...
class ASNBulkEditForm(PrimaryModelBulkEditForm):
    ...
    model = ASN
    fieldsets = (FieldSet('sites', 'rir', 'tenant', 'description'),)
    nullable_fields = ('tenant', 'description', 'comments')
...
class AggregateBulkEditForm(PrimaryModelBulkEditForm):
    rir = DynamicModelChoiceField(queryset=RIR.objects.all(), required=False, label=_('RIR'))
    tenant = DynamicModelChoiceField(label=_('Tenant'), queryset=Tenant.objects.all(), required=False)
    date_added = forms.DateField(label=_('Date added'), required=False)

    model = Aggregate
    fieldsets = (FieldSet('rir', 'tenant', 'date_added', 'description'),)
...
class RoleBulkEditForm(OrganizationalModelBulkEditForm):
    ...
    model = Role
    fieldsets = (FieldSet('weight', 'description'),)
    nullable_fields = ('description', 'comments')
...
class PrefixBulkEditForm(ScopedBulkEditForm, PrimaryModelBulkEditForm):
    ...
    prefix_length = forms.IntegerField(
        label=_('Prefix length'), min_value=PREFIX_LENGTH_MIN, max_value=PREFIX_LENGTH_MAX, required=False
    )
    tenant = DynamicModelChoiceField(label=_('Tenant'), queryset=Tenant.objects.all(), required=False)
    status = forms.ChoiceField(label=_('Status'), choices=add_blank_choice(PrefixStatusChoices), required=False)
    role = DynamicModelChoiceField(label=_('Role'), queryset=Role.objects.all(), required=False)
    is_pool = forms.NullBooleanField(required=False, widget=BulkEditNullBooleanSelect(), label=_('Is a pool'))
    mark_utilized = forms.NullBooleanField(
        required=False, widget=BulkEditNullBooleanSelect(), label=_('Treat as fully utilized')
    )
...
class IPRangeBulkEditForm(PrimaryModelBulkEditForm):
    vrf = DynamicModelChoiceField(queryset=VRF.objects.all(), required=False, label=_('VRF'))
    tenant = DynamicModelChoiceField(label=_('Tenant'), queryset=Tenant.objects.all(), required=False)
    status = forms.ChoiceField(label=_('Status'), choices=add_blank_choice(IPRangeStatusChoices), required=False)
    role = DynamicModelChoiceField(label=_('Role'), queryset=Role.objects.all(), required=False)
    mark_populated = forms.NullBooleanField(
        required=False, widget=BulkEditNullBooleanSelect(), label=_('Treat as populated')
    )
    mark_utilized = forms.NullBooleanField(
        required=False, widget=BulkEditNullBooleanSelect(), label=_('Treat as fully utilized')
    )
    ...
    model = IPRange
    fieldsets = (FieldSet('status', 'role', 'vrf', 'tenant', 'mark_utilized', 'description'),)
    nullable_fields = (
        'vrf',
        'tenant',
        'role',
...
class IPAddressBulkEditForm(PrimaryModelBulkEditForm):
    ...
    mask_length = forms.IntegerField(
        label=_('Mask length'), min_value=IPADDRESS_MASK_LENGTH_MIN, max_value=IPADDRESS_MASK_LENGTH_MAX, required=False
    )
    tenant = DynamicModelChoiceField(label=_('Tenant'), queryset=Tenant.objects.all(), required=False)
    status = forms.ChoiceField(label=_('Status'), choices=add_blank_choice(IPAddressStatusChoices), required=False)
    role = forms.ChoiceField(label=_('Role'), choices=add_blank_choice(IPAddressRoleChoices), required=False)
...
class FHRPGroupBulkEditForm(PrimaryModelBulkEditForm):
    protocol = forms.ChoiceField(
        label=_('Protocol'), choices=add_blank_choice(FHRPGroupProtocolChoices), required=False
    )
    group_id = forms.IntegerField(min_value=0, required=False, label=_('Group ID'))
    auth_type = forms.ChoiceField(
        choices=add_blank_choice(FHRPGroupAuthTypeChoices), required=False, label=_('Authentication type')
    )
    auth_key = forms.CharField(max_length=255, required=False, label=_('Authentication key'))
...
class VLANGroupBulkEditForm(OrganizationalModelBulkEditForm):
    ...
    vid_ranges = NumericRangeArrayField(label=_('VLAN ID ranges'), required=False)
    tenant = DynamicModelChoiceField(label=_('Tenant'), queryset=Tenant.objects.all(), required=False)
...
class VLANBulkEditForm(PrimaryModelBulkEditForm):
    ...
    site_group = DynamicModelChoiceField(label=_('Site group'), queryset=SiteGroup.objects.all(), required=False)
    ...
    group = DynamicModelChoiceField(
        label=_('Group'), queryset=VLANGroup.objects.all(), required=False, query_params={'site_id': '$site'}
    )
    tenant = DynamicModelChoiceField(label=_('Tenant'), queryset=Tenant.objects.all(), required=False)
    status = forms.ChoiceField(label=_('Status'), choices=add_blank_choice(VLANStatusChoices), required=False)
    role = DynamicModelChoiceField(label=_('Role'), queryset=Role.objects.all(), required=False)
    qinq_role = forms.ChoiceField(label=_('Q-in-Q role'), choices=add_blank_choice(VLANQinQRoleChoices), required=False)
...
class VLANTranslationRuleBulkEditForm(NetBoxModelBulkEditForm):
    policy = DynamicModelChoiceField(label=_('Policy'), queryset=VLANTranslationPolicy.objects.all(), selector=True)
    local_vid = forms.IntegerField(required=False)
    remote_vid = forms.IntegerField(required=False)

    model = VLANTranslationRule
    fieldsets = (FieldSet('policy', 'local_vid', 'remote_vid'),)
...
class ServiceTemplateBulkEditForm(PrimaryModelBulkEditForm):
    ...
    fieldsets = (FieldSet('protocol', 'ports', 'description'),)
    nullable_fields = ('description', 'comments')
