...
class ASNRangeQuerySet(RestrictedQuerySet):
    def annotate_asn_counts(self):
        ...
        asns = (
            ASN.objects.filter(asn__gte=OuterRef('start'), asn__lte=OuterRef('end'))
            .order_by()
            .annotate(_=Value(1))
            .values('_')
            .annotate(c=Count('*'))
...
class VLANQuerySet(RestrictedQuerySet):
    def get_for_site(self, site):
        ...
        q = Q()
        q |= Q(scope_type=ContentType.objects.get_by_natural_key('dcim', 'site'), scope_id=site.pk)
    ...
    def get_for_device(self, device):
        ...
        q |= Q(scope_type=ContentType.objects.get_by_natural_key('dcim', 'site'), scope_id=device.site_id)
        if device.location:
            q |= Q(
        ...
        if device.rack:
            q |= Q(scope_type=ContentType.objects.get_by_natural_key('dcim', 'rack'), scope_id=device.rack_id)
    ...
    def get_for_virtualmachine(self, vm):
        ...
        if vm.cluster:
            ...
            q |= Q(
                scope_type=ContentType.objects.get_by_natural_key('virtualization', 'cluster'), scope_id=vm.cluster_id
            )
            if vm.cluster.group:
                q |= Q(
            ...
            if vm.cluster.scope_type == ContentType.objects.get_by_natural_key('dcim', 'location'):
                ...
                q |= Q(scope_type=ContentType.objects.get_by_natural_key('dcim', 'site'), scope_id=vm.cluster.scope.pk)
