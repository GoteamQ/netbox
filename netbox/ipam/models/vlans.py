...
def default_vid_ranges():
    return [NumericRange(VLAN_VID_MIN, VLAN_VID_MAX, bounds='[]')]
...
class VLANGroup(OrganizationalModel):
    ...
    scope_type = models.ForeignKey(to='contenttypes.ContentType', on_delete=models.CASCADE, blank=True, null=True)
    ...
    vid_ranges = ArrayField(IntegerRangeField(), verbose_name=_('VLAN ID ranges'), default=default_vid_ranges)
    tenant = models.ForeignKey(
        to='tenancy.Tenant', on_delete=models.PROTECT, related_name='vlan_groups', blank=True, null=True
    )
    _total_vlan_ids = models.PositiveBigIntegerField(default=VLAN_VID_MAX - VLAN_VID_MIN + 1)
    ...
    class Meta:
        ...
        constraints = (
            models.UniqueConstraint(
                fields=('scope_type', 'scope_id', 'name'), name='%(app_label)s_%(class)s_unique_scope_name'
            ),
            models.UniqueConstraint(
                fields=('scope_type', 'scope_id', 'slug'), name='%(app_label)s_%(class)s_unique_scope_slug'
            ),
        )
...
class VLAN(PrimaryModel):
    ...
    qinq_svlan = models.ForeignKey(
        to='self', on_delete=models.PROTECT, related_name='qinq_cvlans', blank=True, null=True
    )
    ...
    class Meta:
        ...
        constraints = (
            models.UniqueConstraint(fields=('group', 'vid'), name='%(app_label)s_%(class)s_unique_group_vid'),
            models.UniqueConstraint(fields=('group', 'name'), name='%(app_label)s_%(class)s_unique_group_name'),
            models.UniqueConstraint(fields=('qinq_svlan', 'vid'), name='%(app_label)s_%(class)s_unique_qinq_svlan_vid'),
            models.UniqueConstraint(
                fields=('qinq_svlan', 'name'), name='%(app_label)s_%(class)s_unique_qinq_svlan_name'
            ),
        )
        verbose_name = _('VLAN')
    ...
    def clean(self):
        ...
        if self.group and self.site and self.group.scope_type == ContentType.objects.get_for_model(Site):
            if self.site != self.group.scope:
                raise ValidationError(
                    _('VLAN is assigned to group {group} (scope: {scope}); cannot also assign to site {site}.').format(
                        group=self.group, scope=self.group.scope, site=self.site
                    )
                )
        ...
        if self.group:
            if not any([self.vid in r for r in self.group.vid_ranges]):
                raise ValidationError(
                    {
                        'vid': _('VID must be in ranges {ranges} for VLANs in group {group}').format(
                            ranges=ranges_to_string(self.group.vid_ranges), group=self.group
                        )
                    }
                )
        ...
        if self.qinq_svlan and self.qinq_role != VLANQinQRoleChoices.ROLE_CUSTOMER:
            raise ValidationError({'qinq_svlan': _('Only Q-in-Q customer VLANs maybe assigned to a service VLAN.')})
        ...
        if self.qinq_role == VLANQinQRoleChoices.ROLE_CUSTOMER and not self.qinq_svlan:
            raise ValidationError({'qinq_role': _('A Q-in-Q customer VLAN must be assigned to a service VLAN.')})
...
class VLANTranslationRule(NetBoxModel):
    ...
    prerequisite_models = ('ipam.VLANTranslationPolicy',)
    ...
    class Meta:
        ...
        constraints = (
            models.UniqueConstraint(
                fields=('policy', 'local_vid'), name='%(app_label)s_%(class)s_unique_policy_local_vid'
            ),
            models.UniqueConstraint(
                fields=('policy', 'remote_vid'), name='%(app_label)s_%(class)s_unique_policy_remote_vid'
            ),
        )
