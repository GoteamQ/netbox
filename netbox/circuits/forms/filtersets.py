...
class ProviderAccountFilterForm(ContactModelFilterForm, PrimaryModelFilterSetForm):
    ...
    account = forms.CharField(label=_('Account'), required=False)
    tag = TagFilterField(model)
...
class ProviderNetworkFilterForm(PrimaryModelFilterSetForm):
    ...
    provider_id = DynamicModelMultipleChoiceField(queryset=Provider.objects.all(), required=False, label=_('Provider'))
    service_id = forms.CharField(label=_('Service ID'), max_length=100, required=False)
    tag = TagFilterField(model)
...
class CircuitFilterForm(TenancyFilterForm, ContactModelFilterForm, PrimaryModelFilterSetForm):
    ...
    status = forms.MultipleChoiceField(label=_('Status'), choices=CircuitStatusChoices, required=False)
    region_id = DynamicModelMultipleChoiceField(queryset=Region.objects.all(), required=False, label=_('Region'))
    site_group_id = DynamicModelMultipleChoiceField(
        queryset=SiteGroup.objects.all(), required=False, label=_('Site group')
    )
    ...
    commit_rate = forms.IntegerField(
        ...
        widget=NumberWithOptions(options=CircuitCommitRateChoices),
    )
...
class CircuitTerminationFilterForm(NetBoxModelFilterSetForm):
    ...
    location_id = DynamicModelMultipleChoiceField(queryset=Location.objects.all(), required=False, label=_('Location'))
    circuit_id = DynamicModelMultipleChoiceField(queryset=Circuit.objects.all(), required=False, label=_('Circuit'))
    term_side = forms.MultipleChoiceField(label=_('Term Side'), choices=CircuitTerminationSideChoices, required=False)
    provider_network_id = DynamicModelMultipleChoiceField(
        queryset=ProviderNetwork.objects.all(),
        required=False,
        query_params={'provider_id': '$provider_id'},
...
class CircuitGroupAssignmentFilterForm(NetBoxModelFilterSetForm):
    ...
    provider_id = DynamicModelMultipleChoiceField(queryset=Provider.objects.all(), required=False, label=_('Provider'))
    member_id = DynamicModelMultipleChoiceField(queryset=Circuit.objects.all(), required=False, label=_('Circuit'))
    group_id = DynamicModelMultipleChoiceField(queryset=CircuitGroup.objects.all(), required=False, label=_('Group'))
    priority = forms.MultipleChoiceField(label=_('Priority'), choices=CircuitPriorityChoices, required=False)
    tag = TagFilterField(model)
...
class VirtualCircuitFilterForm(TenancyFilterForm, ContactModelFilterForm, PrimaryModelFilterSetForm):
    ...
    type_id = DynamicModelMultipleChoiceField(
        queryset=VirtualCircuitType.objects.all(), required=False, label=_('Type')
    )
    status = forms.MultipleChoiceField(label=_('Status'), choices=CircuitStatusChoices, required=False)
    tag = TagFilterField(model)
...
class VirtualCircuitTerminationFilterForm(NetBoxModelFilterSetForm):
    ...
    virtual_circuit_id = DynamicModelMultipleChoiceField(
        queryset=VirtualCircuit.objects.all(), required=False, label=_('Virtual circuit')
    )
    role = forms.MultipleChoiceField(label=_('Role'), choices=VirtualCircuitTerminationRoleChoices, required=False)
    provider_network_id = DynamicModelMultipleChoiceField(
        queryset=ProviderNetwork.objects.all(),
        required=False,
        query_params={'provider_id': '$provider_id'},
