...
class UserBulkEditForm(BulkEditForm):
    ...
    model = User
    fieldsets = (FieldSet('first_name', 'last_name', 'is_active', 'is_superuser'),)
    nullable_fields = ('first_name', 'last_name')


class GroupBulkEditForm(BulkEditForm):
    pk = forms.ModelMultipleChoiceField(queryset=Group.objects.all(), widget=forms.MultipleHiddenInput)
...
class ObjectPermissionBulkEditForm(BulkEditForm):
    ...
    fieldsets = (FieldSet('enabled', 'description'),)
    nullable_fields = ('description',)
...
class TokenBulkEditForm(BulkEditForm):
    ...
    enabled = forms.NullBooleanField(required=False, widget=BulkEditNullBooleanSelect, label=_('Enabled'))
    write_enabled = forms.NullBooleanField(required=False, widget=BulkEditNullBooleanSelect, label=_('Write enabled'))
    description = forms.CharField(max_length=200, required=False, label=_('Description'))
    expires = forms.DateTimeField(required=False, widget=DateTimePicker(), label=_('Expires'))
    allowed_ips = SimpleArrayField(
        base_field=IPNetworkFormField(validators=[prefix_validator]), required=False, label=_('Allowed IPs')
    )

    model = Token
    fieldsets = (FieldSet('enabled', 'write_enabled', 'description', 'expires', 'allowed_ips'),)
    nullable_fields = (
        'expires',
        'description',
        'allowed_ips',
...
class OwnerGroupBulkEditForm(BulkEditForm):
    pk = forms.ModelMultipleChoiceField(queryset=OwnerGroup.objects.all(), widget=forms.MultipleHiddenInput)
    description = forms.CharField(label=_('Description'), max_length=200, required=False)
...
class OwnerBulkEditForm(BulkEditForm):
    pk = forms.ModelMultipleChoiceField(queryset=Owner.objects.all(), widget=forms.MultipleHiddenInput)
    group = DynamicModelChoiceField(label=_('Group'), queryset=OwnerGroup.objects.all(), required=False)
    description = forms.CharField(label=_('Description'), max_length=200, required=False)

    model = Owner
    fieldsets = (FieldSet('group', 'description'),)
    nullable_fields = (
        'group',
        'description',
    )
