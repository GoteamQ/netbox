...
class Region(ContactsMixin, NestedGroupModel):
    ...
    prefixes = GenericRelation(
        to='ipam.Prefix', content_type_field='scope_type', object_id_field='scope_id', related_query_name='region'
    )
    vlan_groups = GenericRelation(
        to='ipam.VLANGroup', content_type_field='scope_type', object_id_field='scope_id', related_query_name='region'
    )
...
class SiteGroup(ContactsMixin, NestedGroupModel):
    ...
    prefixes = GenericRelation(
        to='ipam.Prefix', content_type_field='scope_type', object_id_field='scope_id', related_query_name='site_group'
    )
    vlan_groups = GenericRelation(
        to='ipam.VLANGroup',
    ...
    def get_site_count(self):
        return Site.objects.filter(Q(group=self) | Q(group__in=self.get_descendants())).count()
...
class Site(ContactsMixin, ImageAttachmentsMixin, PrimaryModel):
    ...
    status = models.CharField(
        verbose_name=_('status'), max_length=50, choices=SiteStatusChoices, default=SiteStatusChoices.STATUS_ACTIVE
    )
    region = models.ForeignKey(to='dcim.Region', on_delete=models.SET_NULL, related_name='sites', blank=True, null=True)
    group = models.ForeignKey(
        to='dcim.SiteGroup', on_delete=models.SET_NULL, related_name='sites', blank=True, null=True
    )
    tenant = models.ForeignKey(
        to='tenancy.Tenant', on_delete=models.PROTECT, related_name='sites', blank=True, null=True
    )
    facility = models.CharField(
        verbose_name=_('facility'), max_length=50, blank=True, help_text=_('Local facility ID or description')
    )
    asns = models.ManyToManyField(to='ipam.ASN', related_name='sites', blank=True)
    time_zone = TimeZoneField(blank=True, null=True)
    physical_address = models.CharField(
        verbose_name=_('physical address'), max_length=200, blank=True, help_text=_('Physical location of the building')
    )
    ...
    # Generic relations
    prefixes = GenericRelation(
        to='ipam.Prefix', content_type_field='scope_type', object_id_field='scope_id', related_query_name='site'
    )
    vlan_groups = GenericRelation(
        to='ipam.VLANGroup', content_type_field='scope_type', object_id_field='scope_id', related_query_name='site'
    )
...
class Location(ContactsMixin, ImageAttachmentsMixin, NestedGroupModel):
    ...
    site = models.ForeignKey(to='dcim.Site', on_delete=models.CASCADE, related_name='locations')
    status = models.CharField(
    ...
    tenant = models.ForeignKey(
        to='tenancy.Tenant', on_delete=models.PROTECT, related_name='locations', blank=True, null=True
    )
    facility = models.CharField(
        verbose_name=_('facility'), max_length=50, blank=True, help_text=_('Local facility ID or description')
    )
    ...
    # Generic relations
    prefixes = GenericRelation(
        to='ipam.Prefix', content_type_field='scope_type', object_id_field='scope_id', related_query_name='location'
    )
    vlan_groups = GenericRelation(
        to='ipam.VLANGroup', content_type_field='scope_type', object_id_field='scope_id', related_query_name='location'
    )
    ...
    class Meta:
        ordering = ['site', 'name']
        constraints = (
            models.UniqueConstraint(fields=('site', 'parent', 'name'), name='%(app_label)s_%(class)s_parent_name'),
            models.UniqueConstraint(
                fields=('site', 'name'),
                name='%(app_label)s_%(class)s_name',
                condition=Q(parent__isnull=True),
    ...
    def clean(self):
        ...
        if self.parent and self.parent.site != self.site:
            raise ValidationError(
                _('Parent location ({parent}) must belong to the same site ({site}).').format(
