...
class PowerPanelTable(ContactsColumnMixin, PrimaryModelTable):
    ...
    powerfeed_count = columns.LinkedCountColumn(
        viewname='dcim:powerfeed_list', url_params={'power_panel_id': 'pk'}, verbose_name=_('Power Feeds')
    )
    tags = columns.TagColumn(url_name='dcim:powerpanel_list')
...
class PowerFeedTable(TenancyColumnsMixin, CableTerminationTable, PrimaryModelTable):
    name = tables.Column(verbose_name=_('Name'), linkify=True)
    power_panel = tables.Column(verbose_name=_('Power Panel'), linkify=True)
    ...
    max_utilization = tables.TemplateColumn(verbose_name=_('Max Utilization'), template_code='{{ value }}%')
    ...
    tags = columns.TagColumn(url_name='dcim:powerfeed_list')
