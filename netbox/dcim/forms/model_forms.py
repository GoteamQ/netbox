...
class RackForm(TenancyForm, PrimaryModelForm):
    ...
    fieldsets = (
        FieldSet(
            'site', 'location', 'name', 'status', 'role', 'rack_type', 'description', 'airflow', 'tags', name=_('Rack')
        ),
...
class DeviceBayTemplateForm(ComponentTemplateForm):
    fieldsets = (FieldSet('device_type', 'name', 'label', 'description'),)
...
class InterfaceForm(InterfaceCommonForm, ModularDeviceComponentForm):
    ...
    wwn = forms.CharField(empty_value=None, required=False, label=_('WWN'))
